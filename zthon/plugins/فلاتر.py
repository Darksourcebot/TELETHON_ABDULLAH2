...
@zedub.zed_cmd(
    ...
    info={
        "header": "لعـرض قائمـة خطـوط عبد الاله ",
        "الاستخـدام": "{tr}.الخطوط",
    },
)
async def lang(event):
    "لعـرض قائمـة خطـوط عبد الاله "
    input_str = event.pattern_match.group(1)
    if not input_str:
        await event.edit(f"**⎉╎قائمـة خطـوط عبد الاله  هـي :-**\n**قم بنسخ اسم الخط ثم ارسل (.خط + اسم الخط)**\n\n{FONTS}")
        return
    else:
        return
...
async def lang(event):
    ...
    if input_str not in font_list:
        ...
        await catevent.edit(f"**⎉╎قائمـة خطـوط عبد الاله>  هـي :-**\n\n{FONTS}")
    else:
